"use client";

import React, { useState, useMemo } from "react";
import { Card, CardHeader, CardBody, Button, Input, Select, SelectItem, Table, TableHeader, TableColumn, TableBody, TableRow, TableCell, Tooltip, Switch } from "@nextui-org/react";

const DeleteIcon = (props) => <svg aria-hidden="true" fill="none" focusable="false" height="1em" role="presentation" viewBox="0 0 20 20" width="1em" {...props}><path d="M10 11V12M10 8V9M19 10a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" /></svg>;

export default function CotizacionProducts({ quoteId, quoteStatus, initialProducts, productCatalog, onUpdate, descuento, comision, proteccion }) {
    const [newProductForm, setNewProductForm] = useState({
        idProducto: "",
        cantidad: 1,
        alto: "",
        ancho: "",
        margen: "",
        usarMargen: false
    });

    const selectedProductInfo = useMemo(() => {
        return productCatalog.find(p => p.id == newProductForm.idProducto);
    }, [newProductForm.idProducto, productCatalog]);

    const { precioUnitario, subtotal: newProductSubtotal } = useMemo(() => {
        if (!selectedProductInfo) return { precioUnitario: 0, subtotal: 0 };

        let precio = parseFloat(selectedProductInfo.precio) || 0;

        if (newProductForm.usarMargen) {
            const costo = parseFloat(selectedProductInfo.costo) || 0;
            const margen = parseFloat(newProductForm.margen);
            if (costo > 0 && margen >= 0 && margen < 100) {
                precio = costo / (1 - (margen / 100));
            } else {
                precio = 0; // Si el margen o costo no son válidos, el precio es 0 para evitar errores
            }
        }

        const comisionPorcentaje = parseFloat(comision);
        if (!isNaN(comisionPorcentaje) && comisionPorcentaje > 0) {
            // Aumenta el precio base en el porcentaje de la comisión
            precio = precio * (1 + comisionPorcentaje / 100);
        }

        const cantidad = parseInt(newProductForm.cantidad) || 1;
        let subtotal = 0;
        if (selectedProductInfo.tipo === 'Telas') {
            const alto = parseFloat(newProductForm.alto) || 0;
            const ancho = parseFloat(newProductForm.ancho) || 0;
            subtotal = (alto * ancho * precio) * cantidad;
        } else {
            subtotal = precio * cantidad;
        }

        return { precioUnitario: precio, subtotal };
    }, [newProductForm, selectedProductInfo, comision]);

    const totals = useMemo(() => {
        let subtotalInflado = 0;
        let descuentoTotal = 0;
        let totalReal = 0;

        initialProducts.forEach((item) => {
            const itemInfo = productCatalog.find(p => p.id === item.idProducto) || {};
            const costoTotalProducto = (itemInfo.tipo === 'Telas')
                ? (item.alto * item.ancho * (item.actual_costo || 0)) * item.cantidad
                : (item.actual_costo || 0) * item.cantidad;

            const proteccionMonto = costoTotalProducto * (parseFloat(proteccion) / 100 || 0);
            const costoConProteccion = costoTotalProducto + proteccionMonto;

            const descuentoMontoProducto = costoConProteccion * (parseFloat(descuento) / 100 || 0);
            const subtotalProductoInflado = costoConProteccion + descuentoMontoProducto;

            subtotalInflado += subtotalProductoInflado;
            descuentoTotal += descuentoMontoProducto;
            totalReal += costoConProteccion;
        });

        return {
            subtotal: subtotalInflado,
            descuentoMonto: descuentoTotal,
            total: totalReal,
        };
    }, [initialProducts, productCatalog, proteccion, descuento]);

    const handleAddProduct = async (e) => {
        e.preventDefault();
        if (!selectedProductInfo) return;

        const productData = {
            idProducto: newProductForm.idProducto,
            cantidad: newProductForm.cantidad,
            alto: newProductForm.alto,
            ancho: newProductForm.ancho,
            actual_precio: precioUnitario,
            actual_costo: selectedProductInfo.costo,
            margen: newProductForm.margen
        };

        await fetch(`/api/cotizacion/${quoteId}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(productData) });
        setNewProductForm({ idProducto: "", cantidad: 1, alto: "", ancho: "", margen: "", usarMargen: false });
        onUpdate();
    };

    const handleDeleteProduct = async (idProductOv) => {
        if (confirm("¿Seguro que quieres eliminar este producto?")) {
            await fetch(`/api/cotizacion/${quoteId}`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ idProductOv }) });
            onUpdate();
        }
    };

    const canAddProducts = quoteStatus !== 'Autorizado' && quoteStatus !== 'Cancelado';

    return (
        <>
            {/* Formulario para Añadir Productos (Condicional) */}
            {canAddProducts && (
                <Card>
                    <CardHeader><h2 className="text-xl font-bold">Añadir Producto</h2></CardHeader>
                    <CardBody>
                        <form onSubmit={handleAddProduct} className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
                                <Select
                                    label="Producto"
                                    className="md:col-span-2"
                                    items={productCatalog}
                                    onChange={(e) => {
                                        const selected = productCatalog.find(p => p.id == e.target.value);
                                        setNewProductForm(p => ({ ...p, idProducto: e.target.value, alto: '', ancho: '', margen: selected?.margen || '' }));
                                    }}
                                    isRequired
                                >
                                    {(prod) => <SelectItem key={prod.id} textValue={prod.nombre}>{prod.nombre} ({prod.tipo})</SelectItem>}
                                </Select>
                                <Input label="Cantidad" type="number" min="1" value={newProductForm.cantidad} onChange={(e) => setNewProductForm(p => ({ ...p, cantidad: e.target.value }))} isRequired />
                                {selectedProductInfo?.tipo === 'Telas' && (
                                    <>
                                        <Input label="Alto (m)" type="number" step="0.01" value={newProductForm.alto} onChange={(e) => setNewProductForm(p => ({ ...p, alto: e.target.value }))} isRequired />
                                        <Input label="Ancho (m)" type="number" step="0.01" value={newProductForm.ancho} onChange={(e) => setNewProductForm(p => ({ ...p, ancho: e.target.value }))} isRequired />
                                    </>
                                )}
                                <div className="flex flex-col gap-2 justify-end h-full md:col-span-1">
                                    <Switch
                                        isSelected={newProductForm.usarMargen}
                                        onValueChange={(isSelected) => setNewProductForm(p => ({ ...p, usarMargen: isSelected, margen: selectedProductInfo?.margen || '' }))}
                                    >
                                        Calcular con Margen
                                    </Switch>
                                    {newProductForm.usarMargen && (
                                        <Input label="Margen" type="number" startContent="%" value={newProductForm.margen} onChange={(e) => setNewProductForm(p => ({ ...p, margen: e.target.value }))} isRequired />
                                    )}
                                </div>
                            </div>
                            <div className="flex justify-end items-center gap-4 mt-4">
                                <p className="text-sm text-default-500">Precio Unitario: ${precioUnitario.toFixed(2)}</p>
                                <p className="font-bold text-lg">Subtotal: ${newProductSubtotal.toFixed(2)}</p>
                                <Button type="submit" color="primary" isDisabled={newProductSubtotal <= 0}>Añadir</Button>
                            </div>
                        </form>
                    </CardBody>
                </Card>
            )}

            {/* Tabla de Productos en la Cotización */}
            <Card>
                <CardHeader><h2 className="text-xl font-bold">Productos en la Cotización</h2></CardHeader>
                <CardBody>
                    <Table
                        removeWrapper
                        aria-label="Tabla de productos en la cotización"
                        bottomContent={
                            <div className="flex flex-col items-end gap-2 p-4 border-t-2 border-default-200">
                                <div className="flex justify-between w-full max-w-xs">
                                    <span className="text-default-500">Subtotal</span>
                                    <span>${totals.subtotal.toFixed(2)}</span>
                                </div>
                                <div className="flex justify-between w-full max-w-xs">
                                    <span className="text-danger">Descuento ({descuento}%)</span>
                                    <span className="text-danger">-${totals.descuentoMonto.toFixed(2)}</span>
                                </div>
                                <div className="flex justify-between w-full max-w-xs font-bold text-lg">
                                    <span>Total General</span>
                                    <span>${totals.total.toFixed(2)}</span>
                                </div>
                            </div>
                        }
                    >
                        <TableHeader>
                            <TableColumn>PRODUCTO</TableColumn>
                            <TableColumn>CANT.</TableColumn>
                            <TableColumn>COSTO UNIT.</TableColumn>
                            <TableColumn>COSTO TOTAL</TableColumn>
                            <TableColumn>PROT. ({proteccion || 0}%)</TableColumn>
                            <TableColumn>DESC. ({descuento || 0}%)</TableColumn>
                            <TableColumn>SUBTOTAL</TableColumn>
                            <TableColumn>ACCIONES</TableColumn>
                        </TableHeader>
                        <TableBody items={initialProducts} emptyContent="No hay productos en esta cotización.">
                            {(item) => {
                                const itemInfo = productCatalog.find(p => p.id === item.idProducto) || {};

                                // Costo total del producto
                                const costoTotalProducto = (itemInfo.tipo === 'Telas')
                                    ? (item.alto * item.ancho * (item.actual_costo || 0)) * item.cantidad
                                    : (item.actual_costo || 0) * item.cantidad;

                                // La protección se calcula sobre el costo total
                                const proteccionMonto = costoTotalProducto * (parseFloat(proteccion) / 100 || 0);
                                const costoConProteccion = costoTotalProducto + proteccionMonto;

                                const descuentoMontoProducto = costoConProteccion * (parseFloat(descuento) / 100 || 0);

                                // El subtotal de la línea es costo + protección + descuento (para inflarlo)
                                const subtotalLinea = costoConProteccion + descuentoMontoProducto;

                                return (
                                    <TableRow key={item.id}>
                                        <TableCell>{item.producto_nombre}</TableCell>
                                        <TableCell>{item.cantidad}</TableCell>
                                        <TableCell>${Number(item.actual_costo || 0).toFixed(2)} {itemInfo.tipo === 'Telas' ? '/ m²' : ''}</TableCell>
                                        <TableCell>${costoTotalProducto.toFixed(2)}</TableCell>
                                        <TableCell className="text-blue-600">+${proteccionMonto.toFixed(2)}</TableCell>
                                        <TableCell className="text-success-600">+${descuentoMontoProducto.toFixed(2)}</TableCell>
                                        <TableCell className="font-bold">${subtotalLinea.toFixed(2)}</TableCell>
                                        <TableCell>
                                            <Tooltip content="Eliminar" color="danger">
                                                <Button isIconOnly variant="light" color="danger" onPress={() => handleDeleteProduct(item.id)} isDisabled={!canAddProducts}>
                                                    <DeleteIcon />
                                                </Button>
                                            </Tooltip>
                                        </TableCell>
                                    </TableRow>
                                )
                            }}
                        </TableBody>
                    </Table>
                </CardBody>
            </Card>
        </>
    );
}